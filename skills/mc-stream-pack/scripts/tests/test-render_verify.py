...
frames (checkerboarded for alpha), fail loudly on mismatches, and (stream-pack
extension) transcode an alpha master to a libvpx-vp9 yuva420p WebM via
--transcode-webm and verify the result. ffmpeg-gated; no models, no network,
no downloads."""
import json
import shutil
import subprocess
...
def have_encoder(name):
    ...
    return r.returncode == 0 and any(
        line.split()[1:2] == [name] for line in r.stdout.splitlines() if line.strip())


HAVE_VP9 = have_encoder("libvpx-vp9")
...
class TestRenderVerify(unittest.TestCase):
    ...
    def test_failed_yuva420p_check_on_opaque_file_carries_hint(self):
        r, out = run([str(self.opaque), "--pixfmt", "yuva420p",
                      "--out-dir", str(self._out_dir())])
        self.assertEqual(r.returncode, 1)
        self.assertFalse(out["checks"]["pixfmt"]["pass"])
        self.assertIn("hint", out["checks"]["pixfmt"])
        self.assertIn("libvpx-vp9", out["checks"]["pixfmt"]["hint"])

    def test_transcode_webm_refuses_opaque_input(self):
        webm = Path(self.tmp.name) / "refused" / "out.webm"
        r, out = run([str(self.opaque), "--transcode-webm", str(webm),
                      "--out-dir", str(self._out_dir())])
        self.assertEqual(r.returncode, 2)
        self.assertFalse(out["ok"])
        self.assertIn("no alpha", out["error"])
        self.assertFalse(webm.exists())


@unittest.skipUnless(HAVE_VP9, "ffmpeg libvpx-vp9 encoder not available")
class TestWebmTranscode(unittest.TestCase):
    tmp: tempfile.TemporaryDirectory
    ...
    def setUpClass(cls):
        cls.tmp = tempfile.TemporaryDirectory()
        root = Path(cls.tmp.name)
        cls.prores = root / "master.mov"
        make_fixture(cls.prores, codec_args=["-c:v", "prores_ks", "-profile:v", "4444",
                                             "-pix_fmt", "yuva444p10le"], dur=1)

    @classmethod
    def tearDownClass(cls):
        cls.tmp.cleanup()

    def test_transcode_produces_verified_vp9_alpha_webm(self):
        root = Path(self.tmp.name)
        webm = root / "deliver" / "stinger.webm"
        r, out = run([str(self.prores), "--transcode-webm", str(webm),
                      "--expect-res", "320x180", "--frames", "2",
                      "--out-dir", str(root / "_verify")])
        self.assertEqual(r.returncode, 0, r.stdout + r.stderr)
        self.assertTrue(out["ok"])
        self.assertTrue(webm.is_file())
        self.assertEqual(out["transcoded"], str(webm))
        self.assertEqual(out["input"], str(webm))  # checks ran on the WebM
        # pixfmt expectation defaulted to yuva420p and passed
        self.assertEqual(out["checks"]["pixfmt"]["expected"], "yuva420p")
        self.assertTrue(out["checks"]["pixfmt"]["pass"])
        self.assertEqual(out["probe"]["codec"], "vp9")
        self.assertTrue(out["probe"]["alpha"])
        self.assertTrue(out["checkerboard"])
        self.assertEqual(len(out["frames"]), 2)

    def test_explicit_pixfmt_flag_overrides_webm_default(self):
        root = Path(self.tmp.name)
        webm = root / "deliver" / "override.webm"
        r, out = run([str(self.prores), "--transcode-webm", str(webm),
                      "--pixfmt", "prores4444",
                      "--out-dir", str(root / "_verify2")])
        self.assertEqual(r.returncode, 1)  # a vp9 webm is not prores4444
        self.assertEqual(out["checks"]["pixfmt"]["expected"], "prores4444")
        self.assertFalse(out["checks"]["pixfmt"]["pass"])
