...
3. Workspace check: `uv run {skill-root}/scripts/ensure_workspace.py --workspace <resolved workspace> --check`. Not ready: tell the creator what a bootstrap downloads (venv wheels of several GB, ~340 MB of Kokoro models now, ~5 GB of Hugging Face cache on the first music/sfx run), get their go-ahead, then run it without `--check`. Idempotent: an existing validated workspace (a lab the creator built by hand counts) is used as-is, never rebuilt or duplicated.
