...
- Install on first graphics use, alongside the engine workspace: `npx hyperframes skills update` installs the maintained core set; `npx skills add heygen-com/hyperframes --all --full-depth` loads the whole catalog so the agent knows every capability. Favor them: the installed skills plus the block catalog are the FIRST place to look for any beat, ahead of authoring anything by hand. Record what resolved.
- Refresh, never blindly expand: `npx hyperframes init` refreshes the core set plus whatever is already installed; `npx hyperframes skills check` reports drift and `npx hyperframes skills update` applies it. The harness loads the skills by its own skill resolution; nothing here is specific to one agent.
